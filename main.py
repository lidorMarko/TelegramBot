...
async def handle_donation_callback(update, context):
    ...
    keyboard = _build_review_keyboard(
        ...
        current_target_id=entry.get("target_channel_id") or context.bot_data.get("public_channel_id", ""),
    ...
    if review_message_id:
        try:
            ...
            keyboard = _build_review_keyboard(
                ...
                cross_promo=bool(entry and entry.get("cross_promo")),
                cross_promo_url=context.bot_data.get("cross_promo_url", ""),
                footer_message=bool(entry and entry.get("footer_message")),
                has_footer=bool(context.bot_data.get("footer_message")),
                english_only=context.bot_data.get("english_only", False),
                reroute_options=context.bot_data.get("reroute_options", []),
                current_target_id=(entry.get("target_channel_id") or context.bot_data.get("public_channel_id", "")) if entry else "",
                job_url=(entry.get("job_snapshot", {}).get(Config.FIELD_URL, "") or "") if entry else "",
                donation=bool(entry and entry.get("donation")),
                donation_url=context.bot_data.get("donation_url", ""),
                cta_text=cta_text,
                cta_url=cta_url,
            )
    ...
    if review_message_id:
        try:
            ...
            keyboard = _build_review_keyboard(
                ...
                donation=bool(entry and entry.get("donation")),
                donation_url=context.bot_data.get("donation_url", ""),
                cta_text=(entry.get("cta_text") or "") if entry else "",
                cta_url=(entry.get("cta_url") or "") if entry else "",
            )
            await context.bot.edit_message_reply_markup(
                chat_id=context.bot_data["review_channel_id"],
                message_id=review_message_id,
...
def _build_app(token: str, watcher: MongoWatcher, sender: TelegramSender,
    ...
    if enable_expiry:
        ...
        app.job_queue.run_daily(
            ...
            time=datetime.time(20, 0, 0, tzinfo=_IL),
        )
